...
writemessage(Display *dpy, Window win, int screen)
{
	...
	XGlyphInfo extents, msg_extents;
	...
	if (!XftColorAllocName(dpy, visual, cmap, text_color, &color)) {
		fprintf(stderr, "slock: Unable to load font color \"%s\"\n", text_color);
		count_error++;
		return;
	}
	draw = XftDrawCreate(dpy, win, visual, cmap);
	XftTextExtentsUtf8(dpy, font, (XftChar8 *)" ", 1, &extents);
	tab_size = 8 * extents.xOff;
	...
	XftTextExtentsUtf8(dpy, font, (XftChar8 *)message, line_len, &msg_extents);
	height = (s_height - msg_extents.height)/2 - (k*20)/3;
	width  = (s_width - msg_extents.xOff)/2;
	...
	for (i = j = k = 0; i <= len; i++) {
		...
		if (i == len || message[i] == '\n') {
			...
			XftDrawStringUtf8(draw, &color, font, width + tab_size*tab_replace, height + 20*k, (XftChar8 *)(message + j), i - j);
       ...
       const char *hash)
{
	...
	if (XSetIMValues(xim, XNDestroyCallback, &imdestroy, NULL))
		fprintf(stderr, "XSetIMValues: "
		                "Could not set XNDestroyCallback.\n");
	xic = XCreateIC(xim, XNInputStyle,
		XIMPreeditNothing | XIMStatusNothing,
		XNClientWindow, locks[0]->win,
	...
	while (running && !XNextEvent(dpy, &ev)) {
		...
		if (ev.type == KeyPress) {
			explicit_bzero(&buf, sizeof(buf));
 			num = XmbLookupString(xic, &ev.xkey, buf, sizeof(buf), &ksym, &status);
			if (IsKeypadKey(ksym)) {
				if (ksym == XK_KP_Enter)
					ksym = XK_Return;
